...
UMBRAL_TEMPERATURA = float(os.getenv('UMBRAL_TEMPERATURA', '26.0'))
TIEMPO_COOLDOWN_ALERTA = int(os.getenv('TIEMPO_COOLDOWN_ALERTA', '1800'))
...
def iniciar_lcd():
    ...
    try:
        ...
        print(f"No LCD screen detected: {e}")
        return False
...
# --- 4. ALERTING (background thread, HTML report) ---
def _enviar_correo_worker(temp, ahora_local):
    """Runs in a background thread so it never blocks the sensor loop."""
    try:
        msg = MIMEMultipart('alternative')
        msg['Subject'] = f"Temperature alert: {sucursal_id}"
        msg['From'] = EMAIL_USER
        msg['To'] = EMAIL_TO
        ...
        <body style="font-family: Arial, sans-serif; color: #333;">
            <div style="max-width: 600px; margin: auto; border: 1px solid #eee; border-radius: 10px; overflow: hidden;">
                <div style="background-color: #d9534f; color: white; padding: 20px; text-align: center;">
                    <h2 style="margin: 0;">Temperature Alert</h2>
                </div>
                <div style="padding: 20px; line-height: 1.6;">
                    <p>An out-of-range temperature was detected at:</p>
                    <div style="background-color: #f9f9f9; padding: 15px; border-radius: 5px; text-align: center;">
                        <span style="font-size: 14px; color: #777;">CURRENT TEMPERATURE</span><br>
                        <span style="font-size: 48px; font-weight: bold; color: #d9534f;">{temp:.1f}&deg;C</span>
                    </div>
                    <table style="width: 100%; margin-top: 20px; border-collapse: collapse;">
                        <tr><td style="padding: 8px; border-bottom: 1px solid #eee;"><strong>Branch:</strong></td><td>{sucursal_id}</td></tr>
                        <tr><td style="padding: 8px; border-bottom: 1px solid #eee;"><strong>Threshold:</strong></td><td>{UMBRAL_TEMPERATURA}&deg;C</td></tr>
                        <tr><td style="padding: 8px; border-bottom: 1px solid #eee;"><strong>Time:</strong></td><td>{ahora_local.strftime('%H:%M:%S')}</td></tr>
                    </table>
                </div>
            </div>
        </body>
        ...
        print("Alert email sent successfully (background thread).")
    except Exception as e:
...
def enviar_alerta(temp):
    ...
    # Update the timestamp immediately so a concurrent loop iteration can't
    # spawn a second thread before this one finishes.
    ultimo_envio_alerta = ahora
    ahora_local = datetime.now(ZONA_HORARIA)

    print("ALERT: starting background thread to send report...")
...
if necesita_scroll:
    texto_sucursal += "   *** "
posicion_scroll = 0
print(f"System active for: {sucursal_id}")
    ...
    try:
        ...
        if ahora_unix - ultimo_tiempo_lcd >= INTERVALO_PANTALLA:
            ...
            if temp_actual is not None:
                ...
                # Reset daily min/max/avg counters on a new day
                ...
                if tiene_pantalla and lcd:
                    try:
                        ...
                        lcd.cursor_pos = (2, 0)
                        # Exact 20-char format: "M:xx.x X:xx.x P:xx.x"
        ...
        if ahora_unix - ultimo_tiempo_nube >= INTERVALO_NUBE:
            if temp_actual is not None:
                try:
                    ...
                    print(f"[{ahora_local.strftime('%H:%M:%S')}] Sent to InfluxDB: {temp_actual}C")
                    ultimo_tiempo_nube = ahora_unix
                except Exception as e:
                    print(f"InfluxDB error: {e}")
            else:
                ultimo_tiempo_nube = ahora_unix - 50  # retry soon if there was no reading
